...
class TestManagement(unittest.TestCase):
    ...
    def test_issue_warning(self):
        ...
        self.assertTrue(any(f"Issued warning to {self.subordinate.name} for: {reason}" in m for m in self.supervisor.memory))
        self.assertTrue(any(f"Received warning from {self.supervisor.name} regarding: {reason}" in m for m in self.subordinate.memory))
    ...
    def test_issue_warning_reaches_threshold_sets_poor_performance(self):
        ...
        self.assertTrue(any(expected_supervisor_memory in m for m in self.supervisor.memory))
        self.assertTrue(any(f"Performance automatically set to Poor due to reaching {config.FIRING_WARNING_THRESHOLD} warnings." in m for m in self.subordinate.memory))
    ...
    def test_fire_subordinate(self):
        ...
        self.assertTrue(any(f"Fired {self.subordinate.name} from their job as {original_job}." in m for m in self.supervisor.memory))
        self.assertTrue(any(f"Was fired by {self.supervisor.name} from job {original_job}. Now Unemployed." in m for m in self.subordinate.memory))
    ...
    def test_warning_leniency_kind_supervisor_good_relationship(self):
        ...
        for _ in range(20): # Simulate multiple checks where random chance is involved
            ...
            if any("Issued warning" in m for m in self.supervisor.memory):
                warned = True; break
            # Reset goal if supervisor idled after not warning, to allow re-evaluation in next loop iter
            if self.supervisor.current_goal.type == GoalType.IDLE:
