...
class Character:
    ...
    def decide_action(self, world: 'World'):
        ...
        if self.job == "Master Craftsman":
            if self.current_goal not in ["Assess Production Needs"] or self.current_goal in ["Wander", None, "Idle"]:
                self.current_goal = "Assess Production Needs"
        ...
        elif self.job == "Expedition Leader":
        ...
        if self.current_goal == "Assess Production Needs":
            ...
            for item_name, target_qty in self.managed_item_targets.items():
                ...
                if pending_or_approved_count < target_qty:
                    ...
                    # Calculate total resources for the order quantity
                    ...
                    print(f"{self.name} (Master Craftsman) generated new Work Order for {qty_to_order} {item_name}(s) (Total Res: {total_req_res_for_order}).")
            ...
            if required_resources:
                for resource, req_qty in required_resources.items(): # req_qty is now total for the order
                    ...
                    if not stockpiles_holding_resource and req_qty > 0: pass
                        ...
                        for sp_name in stockpiles_holding_resource.keys():
                            ...
                            if last_update_day is not None and world.game_time.current_day - last_update_day > STALE_THRESHOLD_DAYS:
                                data_freshness_concerns = True; stale_note = f"Ledger data for {sp_name} (has {resource}) is stale (Day {last_update_day} vs {world.game_time.current_day})."
                                if stale_note not in self.memory: self.add_memory(stale_note); print(f"{self.name} (Manager) thinks: {stale_note}")
                    if available_qty < req_qty: can_approve = False; missing_resources_notes.append(f"{resource} (need {req_qty}, has {available_qty})")
            if data_freshness_concerns and not can_approve: print(f"{self.name} (Manager) notes stale data for order {order_to_process.order_id}, and resources confirmed insufficient.")
            elif data_freshness_concerns: print(f"{self.name} (Manager) notes stale data for order {order_to_process.order_id}, proceeding with caution.")
            if can_approve:
                ...
                self.add_memory(f"Denied WO {order_to_process.order_id}. Reason: {order_to_process.denial_reason}"); print(f"{self.name} (Manager) DENIED order {order_to_process.order_id}. Reason: {order_to_process.denial_reason if missing_resources_notes else 'data freshness/other'}")
            return
            ...
            for sp in stockpiles_to_check:
                day=world.ledger.get_stockpile_last_update_day(sp.name)
                if day is None:target_sp=sp;break
                if day<world.game_time.current_day and day<min_day:min_day=day;target_sp=sp
            ...
            if(self.x,self.y)==spot:
                inv=stockpile_obj.inventory.copy()
                world.ledger.update_stockpile_record(stockpile_obj.name,inv,world.game_time.current_day)
            ...
            else:self.move_towards(spot[0],spot[1],world);return
            ...
            sps=[s for s in world.get_stockpiles_for_resource(res)if s.has_space_for(res,1)]
            if not sps:self.current_goal="Wander";return
            sp=sps[0];self.hauling_info["target_stockpile_name"]=sp.name;self.hauling_info["quantity_to_haul"]=qty
            ...
            if(self.x,self.y)==spot:
                ...
                s,qty_add=sp_obj.add_item(res,qty_dep)
                if s and qty_add>0:
                    ...
                    print(f"{self.name} deposited {qty_add} {res} at {sp_obj.name}. Inv: {self.inventory}")
            ...
            job = self.job
            if job == "Expedition Leader": self.current_goal = "Oversee Expedition"
            elif job == "Master Craftsman": self.current_goal = "Assess Production Needs"
            elif job == "Manager": self.current_goal = "Manage Work Orders"
            elif job == "Bookkeeper": self.current_goal = "Maintain Ledger"
            elif job == "Woodcutter": self.current_goal = "Perform Woodcutter Duties"
            elif job == "Stonemason": self.current_goal = "Perform Stonemason Duties"
